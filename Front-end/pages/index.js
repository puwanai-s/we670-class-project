...
export default function Home() {
    ...
    const fetchData = () => {
        ...
        const fetchLatest = axios.get(config.baseUrl + '/latest?page=' + currentPage);
        axios.all([fetchFeature, fetchLatest]).then(res => {
            setFeatureData(res[0].data);
            setLatestData(res[1].data);
            setLoading(false);
        });
    }
                ...
                <div className='container'>
                    <div className='row'>
                        <div className='col-md-9'>
                            <h2>ฟู๊ดบล็อก ล่าสุด</h2>
                            <div className='mt-3 latest'>
                                <div className='row'>
                                    {
                                        latestData.data?.map((d, index) => {
                                            return (
                                ...
                                <Paginage
                                    ...
                                    pageSize={latestData.totalPages}
                                />
                            </div>
                        </div>
